...
function declarationFooter(
  ...
  const baseText = pl
    ? `Niniejszy dokument zawiera pełny i niezmieniony zapis finansowy rodziny <strong>${escHtml(familyName)}</strong>, wygenerowany przez system Morechard w dniu ${escHtml(date)}. Wszystkie transakcje są niezmienne i zabezpieczone kryptograficznie.`
    : `This document contains the complete and unaltered financial record for family <strong>${escHtml(familyName)}</strong>, generated by Morechard on ${escHtml(date)}. All transactions are cryptographically secured and immutable.`;

...
export async function handleExportJson(request: Request, env: Env): Promise<Response> {
  ...
  return new Response(JSON.stringify(exportPayload, null, 2), {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="moneysteps-export-${family_id}-${Date.now()}.json"`,
    },
  });
}
...
<head>
<meta charset="UTF-8">
<title>MoneySteps — ${t.title} — ${family.name}</title>
<style>
...
<div class="declaration">
  ...
  return new Response(html, {
    ...
    headers: {
      ...
      'Content-Disposition': `inline; filename="moneysteps-report-${family_id}-${Date.now()}.html"`,
    },
  });
}
...
function translations(lang: string) {
  ...
  return {
    ...
    declarationBody: (name: string, date: string) => pl
      ? `Niniejszy dokument zawiera pełny i niezmieniony zapis finansowy rodziny <strong>${escHtml(name)}</strong>, wygenerowany przez system MoneySteps w dniu ${escHtml(date)}. Wszystkie transakcje są niezmienne i zabezpieczone kryptograficznie. Dokument może być przedstawiony jako dowód przed sądem rodzinnym lub mediatorem.`
      : `This document contains the complete and unaltered financial record for family <strong>${escHtml(name)}</strong>, generated by MoneySteps on ${escHtml(date)}. All transactions are cryptographically secured and immutable. This document may be presented as evidence to a UK Family Court or Polish Mediator.`,
  };
}

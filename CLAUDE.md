...
- [x] Per-child governance controls (Approval Mode, Pocket Money Status, Safety Net) — shipped 2026-09-15. The three Manage Family > Child rows that previously just showed a "Coming Soon" toast are now real: **Approval Mode** is a per-child override of the family's `verify_mode` (`users.verify_mode_override`, nullable — NULL inherits the family default); **Pocket Money Status** is a per-child pause flag (`users.allowance_paused`) that the weekly payday sweep (`runPaydaySweep` in `worker/src/index.ts`) now skips, with chores unaffected; **Safety Net** is a per-child override of the family's Global Overdraft Policy (`users.overdraft_enabled_override` / `overdraft_limit_pence_override`). Migration `0098_child_governance_controls.sql`. Approval Mode and Safety Net changes go through the same mutual-consent governance handshake as the existing family-wide `verify_mode` change (`family_governance_log`) — a new parallel `child_governance_log` table plus `worker/src/routes/childControls.ts` (`/api/child-controls/*`, `/api/child-governance/*`) implement the per-child version: a solo parent's change applies immediately (self-confirmed, still logged for the audit trail), a co-parenting family's change creates a pending row the other parent must confirm or reject (`ChildGovernanceBanner.tsx`, mirrors the existing `GovernanceConsentBanner.tsx`). Pocket Money Status is ungated — reversible, no debt/dispute exposure, either parent flips it instantly. The per-child Approval Mode override is threaded through every place that resolves `verify_mode` for a specific child (`worker/src/lib/verifyMode.ts` — `completions.ts` ×2, `finance.ts`, `ledger.ts`, the payday sweep); chore auto-approve was confirmed to ignore `verify_mode` entirely already (always `verified_auto`), so it needed no change. **Known limitation, not addressed by this work**: Safety Net (both the new per-child override and the pre-existing family-wide Global Overdraft Policy it layers on) stores a policy value but nothing in the app enforces it yet — no spend/goal-purchase path checks it. Covered by worker unit tests only (childControls.test.ts, governance.test.ts) — no live end-to-end verification, same `wrangler dev --remote` 503 limitation noted elsewhere in this file.
...
### **Infrastructure**
- [x] Governance endpoint role gate fix — 2026-09-15. Background security review on the new per-child governance work (`childControls.ts`) found that `handleChildGovernanceConfirm`/`Reject`/`Get` checked the caller's family but never their role, so a child's own JWT (same `family_id`, `role: 'child'`) could confirm/reject a pending Approval Mode or Safety Net request. Fixed there, then found and fixed the identical pre-existing gap in the original family-wide flow (`worker/src/routes/governance.ts` — `handleGovernanceRequest`/`Confirm`/`Reject`/`Get`, none of which checked `auth.role`). All eight handlers now gate on `auth.role !== 'parent'`. Covered by `childControls.test.ts` and the new `governance.test.ts`.
- [x] D1 transient-error handling extended — 2026-09-15. The worker already mapped one known transient D1 platform error ("D1 DB storage operation exceeded timeout" — a Durable Object reset) to a 503 that the frontend auto-retries, suppressed from Sentry. A second transient message, Cloudflare's account-wide D1 throughput cap ("...too much load on D1 DBs..."), wasn't matched — every occurrence paged Sentry as a hard error and the client never retried it (only 503s trigger `api.ts`'s retry branch). First seen live 2026-09-15 (`handleGetGiveRequests`, unrelated to that route's own query — just whichever request was in flight when the cap was hit). Extracted the check into `worker/src/lib/transientD1Error.ts` (unit tested) and extended it to match both messages. Noted but not acted on: the app has ~10 independent 30-second polling loops (child/parent dashboard tabs, give-requests panel) all on the same fixed interval with no jitter — a plausible contributor to synchronized load spikes, not fixed here.

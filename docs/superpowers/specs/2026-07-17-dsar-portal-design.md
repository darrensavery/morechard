...
**Known gap, deliberately out of scope for this build:** single-child erasure only sweeps the tables above (chat/learning-lab/gamification). It does **not** touch family-keyed tables where the child also appears — `completions`, `goals`, `spending`, `payouts`, `give_requests`, `jar_movements`, `parent_messages`, etc. — because those are only ever swept by the whole-family purge path today. After the T+30 hard-delete of the child's `users` row, those rows retain a dangling `child_id` reference with no documented retention basis (unlike the ledger, which has an explicit LIA-3 legal basis). Flagged during final review (2026-07-17); accepted as a follow-up rather than expanding this build's purge blast radius without its own dedicated review.
...
**Known gap, deliberately out of scope for this build:** the retry path described above (re-run execution against `processing` rows) was not built. As shipped, any execution-time exception (D1 hiccup, Resend outage, R2 failure) — including one that occurs *after* the erasure itself has already committed but before the final `status = 'completed'` write succeeds — flips the request to `status = 'needs_clarification'` and sends the requester the same clarification copy used for an ambiguous child-name match. In the partial-failure case this is misleading: the requester's data may already be genuinely erased, but they receive a "we couldn't match your child's name" email and no path to confirm completion short of contacting support. There is also no automated retry for any `needs_clarification`/stuck request — it is a manual-support dead end. Flagged during final review (2026-07-17); accepted as a follow-up. A real fix needs a proper retry/reconciliation design (e.g. a distinguishing failure-reason column instead of reusing one status value for three different scenarios), not a quick patch.

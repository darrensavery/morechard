...
5. **Sentry data residency** — **Resolved 2026-07-17.** Confirmed directly in the Sentry dashboard (Organization Settings → Data Storage Region): **European Union**. The LIA (LIA-2) had assumed a US-hosted account and cited "EU SCCs with UK Addendum" as the transfer safeguard — corrected to state plainly that no international transfer occurs for this processor, consistent with the Worker's `ingest.de.sentry.io` (Frankfurt) DSN.
...
1. ~~Resolve the Anthropic/Zoho/Brevo disclosure and Paddle-framing findings above.~~ **Done 2026-07-17** — privacy policy v1.4. ~~Sentry region reconciliation (finding #5).~~ **Done 2026-07-17** — confirmed EU, LIA corrected.

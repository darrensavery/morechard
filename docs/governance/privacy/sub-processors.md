...
**Document type:** Internal accountability record. Should also be published (or made available on request) as a customer/school-facing sub-processor list once reviewed. Not legal advice — should be reviewed by a data-protection specialist, particularly the three newly-identified undisclosed processors below, before being relied upon or published externally.
...
## ⚠️ Findings requiring a decision before this document is finalised or published

A code-level audit for this document (2026-07-17) found **three sub-processors that are live and actively receiving personal data but are not named anywhere in the current privacy policy, DPIA, or LIA**, plus two factual mismatches worth resolving. None of this is a code change — it's a disclosure and documentation gap.

1. **Anthropic (Claude)** — powers the autonomous customer-support agent (`worker/src/lib/agent/claudeClient.ts`). Receives support-ticket text and, via Zoho, the requester's email address, for automated triage and reply drafting. **Not in the privacy policy.**
2. **Zoho Desk** — the support-ticketing platform tickets actually live in (`worker/src/lib/agent/zoho.ts`). Receives ticket subject/description, requester email, and last name. **Not in the privacy policy.**
3. **Brevo (Sendinblue)** — marketing-list email management, separate from Resend's transactional email (`worker/src/routes/consent.ts`). Receives email address + marketing-list membership for consenting parents. **Not in the privacy policy.**
4. **Paddle** is named in the privacy policy and in the DPIA/LIA as "Merchant of Record," but no Paddle integration exists in the codebase — only Stripe is wired up. This looks like stale/aspirational text from an earlier plan; recommend removing the Paddle reference rather than treating it as a real sub-processor.
5. **Sentry data residency** — the LIA (LIA-2) describes the international-transfer safeguard as if Sentry data leaves the UK/EU ("EU SCCs with UK Addendum"), but the actual Sentry DSN wired into the Worker points at `ingest.de.sentry.io` (Frankfurt, Germany — EU). Worth confirming the real account region in the Sentry dashboard and correcting the LIA if it is in fact EU-hosted (a stronger, not weaker, position).

None of items 1–3 need to stop being used — the support-agent design already routes drafts through a human before anything reaches a customer (see `docs/superpowers/specs/2026-07-13-autonomous-support-agent-design.md`), which is the actual safeguard a DPO would want to hear about. But a school reviewer or the ICO would expect every processor touching personal data to be named. Recommend: update `docs/notebooklm/privacy-policy.md` to add Anthropic, Zoho Desk, and Brevo; remove or correct the Paddle reference; confirm and correct the Sentry region in the LIA.
...
| **Brevo (Sendinblue)** — ⚠️ *not yet in the public privacy policy, see finding #3 above* | Email address, marketing-list membership. | Marketing email list management for parents who've opted in to marketing communications, and pre-launch "register your interest" signups. | Not pinned in code. | Best-effort integration — a Brevo failure never blocks the underlying request; not independently confirmed for retention. |
...
1. Resolve the five findings above (Anthropic/Zoho/Brevo disclosure, Paddle removal, Sentry region reconciliation) — ideally before this document or an updated privacy policy is shown to a school.

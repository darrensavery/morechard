...
- **D1 backup / disaster recovery runbook** (`docs/dev/d1-backup-recovery-runbook.md`) — confirmed Cloudflare D1 Time Travel is live on both databases already (automatic, no setup needed — it was undocumented, not missing), with continuous point-in-time bookmarks. Documented the restore procedure (fork-and-verify before restoring in place), RPO (~0, continuous) and RTO (untested — budgeted 15–30min pending a real drill), and the one real gap Time Travel doesn't cover: a deleted D1 *resource* itself has no recovery path. Flagged, not implemented: a scheduled off-platform `wrangler d1 export` to R2 as a true backstop, and an actual restore drill against `morechard-dev` to replace the RTO estimate with a measured number.

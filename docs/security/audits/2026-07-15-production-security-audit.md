...
| # | Finding | Status |
|---|---|---|
| 1 | WebAuthn/biometric check (`app/src/lib/biometrics.ts`) is client-side only — the challenge/signature is never sent to or verified by the worker. Trivially bypassable; presented to users as a real security control. | **Fixed (Pass 7).** Real server-side verification shipped: web via `@simplewebauthn/server`/`browser` (COSE public key, real attestation/assertion verification, signature-counter clone-detection alerting to Sentry with a dedicated `webauthn-clone-detected` fingerprint); native (Capacitor) via a Web-Crypto ECDSA key pair in IndexedDB gated by a native biometric prompt (`@aparajita/capacitor-biometric-auth`), no custom Swift/Kotlin. Both ceremonies unlock the device and re-issue a real session on success — unifies the old "unlock" and "login" concepts. See `docs/superpowers/specs/2026-07-16-webauthn-verification-design.md`. |
...
| 10 | No dedicated alerting for failed Stripe payments — only generic Sentry exception capture. | **Fixed and activated.** Stripe webhook captures failure events (`payment_intent.payment_failed`, `charge.failed`, `invoice.payment_failed`, `checkout.session.expired`/`async_payment_failed`) to Sentry with a dedicated fingerprint (`stripe-payment-failure`). Sentry Alert Rule created in the dashboard (Issues alert: triggers on "a new issue is created" + "a resolved issue becomes unresolved", scoped to production, notifying by email) — same pattern as the existing Cron/Uptime monitors. |
...
| 26 | No load testing / capacity planning doc. | **Fixed (Pass 4 baseline, extended Pass 8 2026-07-16).** `docs/dev/capacity-planning.md` + `worker/scripts/load-test.mjs` (public `/api/health`) + `worker/scripts/load-test-authenticated.mjs` (demo-account `/api/insights` cached path + `/api/export/pdf`). `/api/chat` still untested by design — see finding #10 below and Open item #3. |
...
**Pass 7 (2026-07-16 — WebAuthn server-side verification, finding #1):**
- Brainstormed, designed, and implemented per `docs/superpowers/specs/2026-07-16-webauthn-verification-design.md` and `docs/superpowers/plans/2026-07-16-webauthn-verification.md`.
- `worker/migrations/0087_webauthn_credentials.sql` — new table storing WebAuthn (web) and native-ECDSA (app) public keys, applied to both `morechard-dev` and `morechard` (production).
- `worker/src/lib/webauthn.ts` — base64url encode/decode, native-credential-ID derivation (`SHA-256` of the SPKI public key), raw ECDSA signature verification, KV-backed one-time challenge storage.
- `worker/src/routes/auth.ts` — exported `issueParentJwt`, extracted `issueChildJwt`/`createChildSession` out of `handleChildLogin` so WebAuthn login can issue a real session without duplicating PIN-specific `child_logins`/graduation bookkeeping.
- `worker/src/routes/webauthn.ts` — 4 new endpoints (`register/options`, `register/verify` authenticated; `login/options`, `login/verify` public), wired into `index.ts`. Web path uses `@simplewebauthn/server` (real COSE public key, real attestation/assertion verification, signature-counter clone-detection); a counter regression is rejected (401) and logged to Sentry with a dedicated `webauthn-clone-detected` fingerprint, same pattern as `stripe-payment-failure`. Native path verifies a raw ECDSA signature against a stored SPKI public key — no counter/clone-detection signal on this path (accepted, documented limitation).
- `app/src/lib/webauthnNative.ts` — native ECDSA key pair (Web Crypto, non-extractable) persisted in IndexedDB (same technique as the encrypted bank vault), gated behind a native biometric prompt (`@aparajita/capacitor-biometric-auth`) before every use — no custom Swift/Kotlin.
- `app/src/lib/api.ts` — 4 thin wrapper functions for the new endpoints.
- `app/src/lib/biometrics.ts` — full internal rewrite. All 5 exported function signatures kept byte-for-byte unchanged, so none of its 5 existing call sites (`Stage3SecureApp.tsx`, `JoinFamilyScreen.tsx`, `PinManagementSettings.tsx`, `useGatekeeper.tsx`, `LockScreen.tsx`) needed any change. Every successful verification now both unlocks the device and (re)issues a real session (`primeAuthState()` re-synced after `setToken()`), unifying what used to be two separate concepts.
...
All Pass 7 changes verified: `worker/` — `tsc --noEmit` clean, 364/364 tests passing (including a dedicated isolated test file for the counter-regression/Sentry clone-detection path, added after task review flagged it as the one security-critical behavior with no coverage). `app/` — `tsc -b --noEmit` clean except the pre-existing, out-of-scope `localBankDetails.ts:135` error, 125/125 vitest tests passing. No live device/browser verification was possible in this environment (no iOS/Android device or emulator, `wrangler dev --remote` 503s here) — ships verified by unit tests + code review only; see Open Items #9.

---

**Pass 8 (2026-07-16 — broader zod adoption + authenticated load test, findings #28 and #26/Open items #2-3):**
- **Zod adoption completed across all remaining routes.** All 36 remaining `parseBody()` call sites across `chores.ts`, `completions.ts`, `finance.ts`, `goals.ts`, `market-rates.ts`, `plans.ts`, `reviewPrompt.ts`, `settings.ts`, `suggestions.ts`, plus the last few in `auth.ts`/`invite.ts`, now use `parseValidatedBody()` with per-handler zod schemas, matching the pattern established in Pass 3. Only ad hoc type/presence validation was replaced — DB-lookup-dependent authorization checks (family/child ownership, etc.) were left untouched. Verified: `tsc --noEmit` clean, 364/364 tests passing.
- **Authenticated-route load test** — new `worker/scripts/load-test-authenticated.mjs`, authenticating via the shared demo (Thomson) account (`POST /auth/demo/register`) rather than a seeded test family. Load-tested `GET /api/insights` (cached path, after a priming call) and `GET /api/export/pdf?tier=basic` at 10 connections × 30s each against a real preview URL: zero errors/non-2xx across ~1,300 combined requests. Results in `docs/dev/capacity-planning.md`. `/api/chat` deliberately excluded — see finding #10 below.
- **New finding surfaced (not remediated this pass):** while scoping the chat load test, discovered `/api/chat` has no content-moderation layer on child-facing free-text input/output, and no off-topic-refusal instruction in its system prompt — only a 20-msg/hour rate limit and the model's baseline training. Logged as finding #10 above; user elected to hold off on any code change pending further discussion, so no fix was implemented this pass.

**Pass 9 (2026-07-16, `a0115af` — resolved by removal, not remediation):** the finding-#10 free-text child chat feature was removed entirely rather than patched — `worker/src/routes/chat.ts` and its call sites deleted outright. Closes the content-safety gap by eliminating the surface, not by adding moderation to it.

**Pass 10 (2026-07-17 — closed the stale finding #10 (chat) writeup, re-checked open items #5–9, drilled the incident-response runbook as a tabletop walkthrough):** see the updated "Open items" list below for the per-item results — nothing net-new to fix was found; #8 (tabletop walkthrough) is now done, #5/#6/#7/#9 remain blocked in this environment for the same reasons as before (dashboard-only access, no device/emulator, sandboxed `wrangler dev --remote`), reconfirmed rather than assumed.

## Open items for the next audit pass

Roughly in priority order:
1. ~~Sentry Alert Rule for Stripe payment failures.~~ **Done 2026-07-17** — Issues alert created (new issue + resolved-becomes-unresolved triggers, production-scoped, email notification).
2. ~~Formal load test against a preview URL with real authenticated traffic (chat, insights generation, PDF export).~~ **Done 2026-07-16** for insights (cached path) and PDF export — see `docs/dev/capacity-planning.md`, zero errors across ~1,300 requests at moderate load. `/api/chat` deliberately excluded (see item #10 below) — needs a dedicated test family, not the shared demo account, plus the content-safety question resolved first.
3. ~~Broader zod adoption across the remaining ~28 authenticated-route call sites.~~ **Done 2026-07-16** — all remaining routes converted, see Pass 8 above.
4. Re-run the D1 restore drill periodically (every 6 months, or after any wrangler major-version upgrade) — see `docs/dev/d1-backup-recovery-runbook.md`.
5. Watch Turnstile's pass/fail rate after activation (2026-07-15) — confirm Managed mode isn't creating friction for legitimate parents/children before considering it "done" rather than just "live". **Re-checked 2026-07-17: still dashboard-only** — Turnstile analytics isn't reachable via the CLI's current OAuth scope (`account:read`, `workers:write`, `d1:write`, no analytics scope), so this stays a manual Cloudflare-dashboard check.
6. Delete the old over-scoped Cloudflare API tokens (`Edit Cloudflare Workers` ×2, `moneysteps-deploy`, `moneysteps build token`) once confidence in the new minimal one is fully established — not done immediately after rotation in case of an unexpected rollback need. **Re-checked 2026-07-17: still dashboard-only** — the CLI's OAuth session has no account-API-token-management scope, and deleting live auth credentials isn't something to do without the user's hand on it directly regardless.
7. Live Playwright verification of the new cookie/CSRF flow (`app/e2e/auth-cookie.spec.ts`) — written and statically cross-checked against the route code, but never actually run: `wrangler dev --remote` returns a Cloudflare 503 on every route in the sandboxed dev environment used for Pass 6 (reproduces identically on unmodified `main`, so it's a pre-existing tooling limitation, not a regression). Run it for real the next time a normal local dev setup is available. **Re-attempted 2026-07-17: still 503s** (`GET /api/health` → 503 after ~5s) — confirmed still the same sandbox limitation, not a regression.
8. ~~This incident-response runbook itself hasn't been drilled (unlike the D1 one) — worth at least a tabletop walkthrough.~~ **Done 2026-07-17** — see `docs/dev/infra-incident-response-runbook.md`'s own changelog note; all referenced commands and linked docs verified live (production `/api/health` → 200, `gh run list` and `wrangler versions list --env production` both ran clean, both linked runbooks exist).
9. Live device/browser verification of the WebAuthn work shipped in Pass 7 — real Face ID/Touch ID prompts (native) and a real browser WebAuthn ceremony (web) were never exercised on actual hardware; this environment has no iOS/Android device or emulator and `wrangler dev --remote` doesn't run here (same limitation as item 7). Ships verified by unit tests + code review only — run it for real the next time real hardware and a working local dev setup are both available. **Re-checked 2026-07-17: still blocked** — no `adb`/emulator/device available in this environment.
10. ~~No content-safety layer on child-facing AI Mentor chat.~~ **Resolved by removal, `a0115af` (2026-07-16).** Rather than adding moderation to `POST /api/chat`, the entire unmoderated free-text child chat feature was removed — `worker/src/routes/chat.ts` and its call sites deleted outright. See [[project_ai_mentor_chat_safety]] memory: do not re-add free-text child chat without new legal/escalation scoping.
11. ~~Load-test `/api/chat` itself.~~ **Moot — the route no longer exists** (removed alongside item #10, `a0115af`).

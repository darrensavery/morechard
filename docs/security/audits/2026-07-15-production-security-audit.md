...
2. Formal load test against a preview URL with real authenticated traffic (chat, insights generation, PDF export) — the current baseline only hits the public `/api/health` endpoint.
3. Broader zod adoption across the remaining ~28 authenticated-route call sites, incrementally.

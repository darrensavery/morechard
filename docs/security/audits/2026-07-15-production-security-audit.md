...
1. **Sentry Alert Rule for Stripe payment failures** — code-side capture is done, the dashboard rule isn't created yet.
...
5. Watch Turnstile's pass/fail rate after activation (2026-07-15) — confirm Managed mode isn't creating friction for legitimate parents/children before considering it "done" rather than just "live".
6. Delete the old over-scoped Cloudflare API tokens (`Edit Cloudflare Workers` ×2, `moneysteps-deploy`, `moneysteps build token`) once confidence in the new minimal one is fully established — not done immediately after rotation in case of an unexpected rollback need.
...
9. Live device/browser verification of the WebAuthn work shipped in Pass 7 — real Face ID/Touch ID prompts (native) and a real browser WebAuthn ceremony (web) were never exercised on actual hardware; this environment has no iOS/Android device or emulator and `wrangler dev --remote` doesn't run here (same limitation as item 7). Ships verified by unit tests + code review only — run it for real the next time real hardware and a working local dev setup are both available.
10. **New finding, 2026-07-16 (not yet actioned): no content-safety layer on child-facing AI Mentor chat.** `POST /api/chat` (`worker/src/routes/chat.ts`) sends the child's free-text message (up to 500 chars) directly to OpenAI as a `user`-role turn, with no moderation-endpoint check (input or output) and no explicit off-topic/refusal instruction in the system prompt — the only guardrail is a 20-messages/hour per-child rate limit plus the model's own baseline training. Raised during a conversation about load-testing the endpoint; user elected to investigate/decide separately rather than action immediately. Two options discussed: (a) add an OpenAI moderation-endpoint check on both directions plus a refusal instruction, keeping free-text; (b) replace free-text input with a curated preset-prompt/quick-reply set, eliminating open-ended conversation risk. Neither implemented yet.
11. Load-test `/api/chat` itself — deliberately skipped in the 2026-07-16 pass (see finding #10 and `docs/dev/capacity-planning.md`). Needs a dedicated seeded test family so a real burst doesn't lock the shared demo account's chat out for genuine visitors, and arguably belongs after the content-safety question is resolved, not before.
